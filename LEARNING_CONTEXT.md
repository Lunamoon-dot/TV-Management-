...
Ngày 23/09/2026: bài 44 thêm rate limiting riêng cho POST /api/auth/login bằng middleware có sẵn của ASP.NET Core. Mỗi IP được 10 request/phút; request thứ 11 trả 429 ProblemDetails + Retry-After, React hiện thông báo chờ. Đây là lớp bảo vệ endpoint, khác Identity lockout bảo vệ từng account. Backend build, frontend lint/build/48 test, test rate limit và regression auth-cookie đều đạt; không migration. Khi deploy qua proxy/nhiều instance cần xử lý forwarded headers và limiter ở tầng hạ tầng hoặc distributed.

Ngày 23/09/2026: bài 45 cấu hình Data Protection application name `TvStore`; Production bắt buộc có `DataProtection__KeysPath` tới durable storage và fail fast nếu thiếu. Test publish Production xác nhận thiếu config không start, có config thì health 200 và sinh key XML. Development vẫn dùng key store mặc định; chưa khóa vào cloud provider/certificate cụ thể. Cần bảo vệ quyền filesystem và mã hóa key at rest khi chọn hạ tầng. Không migration.

Ngày 23/09/2026: bài 46 thêm CorrelationIdMiddleware ngoài exception handler. Mọi response có X-Correlation-ID; input client chỉ được giữ khi dài tối đa 64 và thuộc whitelist ASCII, nếu không server sinh GUID. ID được gán vào HttpContext.TraceIdentifier + logging scope; ProblemDetails 500 và header dùng cùng ID. Test health và lỗi SQL thật đạt; không migration. Chưa chọn log aggregation/APM provider.

Ngày 24/09/2026: bài 47 thêm structured business logs chọn lọc vào tạo/replay checkout, reject thiếu product/stock, payment, order transition và cancellation/restock. Không log email, shipping data, reason/note body, password/cookie/token. Console IncludeScopes=true để thấy CorrelationId; EF Database.Command mặc định Warning để giảm nhiễu. Backend build và integration Admin Orders đạt, dữ liệu test đã dọn, không migration.

Ngày 24/09/2026: bài 48 nối correlation ID sang React. shared/api/errors đọc x-correlation-id từ Axios response và ghép mã hỗ trợ; Cart checkout chỉ dùng cho lỗi không dự đoán, giữ UX riêng cho 400/401 và không tạo mã khi lỗi mạng không có response. Frontend lint/build, 10 files/51 tests đạt; backend/DB không đổi.

Ngày 24/09/2026: bài 49 thêm `.github/workflows/ci.yml` cho push main/PR. Ubuntu runner dùng Node22/.NET10, npm ci + lint + 51 tests, Release build, publish ASP.NET+React và kiểm tra DLL/index. Local mô phỏng toàn chuỗi đạt. SQL integration/E2E/CD chưa đưa vào workflow để tránh setup thiếu cô lập; sẽ mở rộng riêng.

Ngày 24/09/2026: bài 50 thêm xUnit project `nothing.Tests` vào solution. Tách OrderStatusPolicy cho transition và quyền cancel Customer/Admin; controller/service dùng policy chung. 12 unit tests đạt ~0,35s, Release solution build sạch và regression Admin Orders HTTP+SQL đạt; CI restore/build solution và chạy dotnet test trước publish. Không migration.

Ngày 24/09/2026: bài 51 thêm Dockerfile multi-stage (Node22 frontend, .NET10 SDK backend, ASP.NET10 non-root runtime port8080), `.dockerignore` và CI docker build. csproj hỗ trợ BuildFrontend=false để Docker không build React hai lần; publish thường vẫn giữ React. Runtime phải nhận connection string + DataProtection__KeysPath=/keys và mount volume. Máy local chưa có Docker; image được xác nhận qua GitHub runner, chưa push registry/deploy.

Ngày 24/09/2026: bài 52 thêm Customer profile backend. ApplicationUser thêm FullName/ShippingAddress và giới hạn PhoneNumber20; migration `20260924060043_AddCustomerProfile` đã apply local sau khi xác nhận 2 user cũ không có phone quá dài. GET/PUT `/api/account/profile` dùng cookie owner, PUT CSRF + validation, dữ liệu trim và UserManager.UpdateAsync. Integration hai account xác nhận isolation/persist SQL và cleanup; 12 unit tests đạt. Frontend profile + checkout prefill là bước kế.
...
Ngày 24/09/2026: bài 53 Customer Profile frontend: thêm route bảo vệ `/account/profile`, email navbar thành link hồ sơ, form React tải/lưu qua Axios + CSRF và Zod validate/trim. Dùng local state thay vì Zustand vì form không chia sẻ toàn app. Frontend 53 test, lint/build đạt; browser test anonymous redirect login. Xem `lessons/53-customer-profile-react.md`. Tiếp theo dùng profile làm mặc định cho checkout nhưng Order vẫn lưu snapshot riêng.

Ngày 24/09/2026: bài 54 prefill checkout: CartPage gọi GET profile khi đã đăng nhập và chỉ điền các ô giao hàng đang trống, nên response chậm không ghi đè dữ liệu user vừa nhập. Checkout vẫn sửa độc lập và Order giữ snapshot; không tự cập nhật profile. Lỗi tải profile không chặn nhập thủ công. Frontend 55 test, lint/build đạt. Xem `lessons/54-prefill-checkout-from-profile.md`.

Ngày 24/09/2026: bài 55 change password backend: PUT `/api/account/password` yêu cầu auth + CSRF, dùng UserManager.ChangePasswordAsync và RefreshSignInAsync. Identity kiểm tra password cũ, policy, hash/security stamp; lỗi map về đúng field. Integration test SQL thật đạt gồm CSRF/sai password/policy/success/cookie refresh/old-new login; account test đã dọn. Xem `lessons/55-change-password-api.md`. Bài tiếp theo làm React form với xác nhận mật khẩu mới và nút hiện/ẩn.

Ngày 25/09/2026: bài 56 change password frontend: form nằm trong `/account/profile`, ba ô có toggle riêng, Zod kiểm tra policy/mật khẩu xác nhận rồi transform bỏ confirm trước request. Axios gửi CSRF tới PUT password, map lỗi backend về field, 401 quay login và success xóa mật khẩu khỏi state. Frontend 57 test, lint/build đạt. Xem `lessons/56-change-password-react.md`.

Ngày 25/09/2026: bài 57 password recovery backend: thêm forgot/reset password anonymous nhưng có CSRF + rate limit 5/15 phút/IP; forgot trả 204 giống nhau để chống user enumeration. AddDefaultTokenProviders, Identity reset token được Base64Url encode. Development ghi message vào `.dev-emails` gitignored; production provider vẫn là việc bắt buộc trước deploy. Integration SQL thật xác nhận CSRF, generic response, delivery, invalid/valid token và old/new login; dữ liệu test đã dọn. Xem `lessons/57-password-recovery-api.md`.

Ngày 25/09/2026: bài 58 password recovery frontend: thêm forgot/reset routes, link từ login, Zod và Axios+CSRF. Forgot giữ thông báo chung chống enumeration; reset đọc email/code một lần rồi replaceState xóa token khỏi URL, confirm chỉ ở client. Development message thêm ResetPath. Frontend 60 test, lint/build và browser states đạt. Xem `lessons/58-password-recovery-react.md`.
